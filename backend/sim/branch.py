...
Three protagonists, each a different fate axis:
...
End-of-game: the LLM reads each protagonist's full arc trail and picks the
most fitting ending. Gate-based heuristics serve as fallback when the LLM is
unavailable.
"""
...
# ending_id -> (title, description, persona_mutation)
...
# Which ending IDs belong to each protagonist type
_TYPE_ENDINGS: dict[str, list[str]] = {
    "panic_seller": ["E1", "E2", "E3", "E4"],
    "whale": ["W1", "W2", "W3", "W4"],
    "fomo_trader": ["F1", "F2", "F3", "F4"],
}
...
# --------------------------------------------------------------------------- #
# Gate functions (fallback when LLM unavailable)
# --------------------------------------------------------------------------- #
...
def _ghost(agent_type: str, s: dict, reached: str) -> str:
    """Nearest un-reached ending -> 'almost' caption."""
...
# --------------------------------------------------------------------------- #
# LLM-based ending selection
# --------------------------------------------------------------------------- #
def _build_llm_prompt(agent: Agent, arc_state: dict, trail: list[dict]) -> str:
    """Build a prompt for the LLM to choose the most fitting ending."""
    endings = _TYPE_ENDINGS[agent.type]
    options = "\n".join(
        f"  {eid}: {ENDING_META[eid][0]} — {ENDING_META[eid][1]}"
        for eid in endings
    )

    # Summarize the round-by-round trail
    trail_lines = []
    for t in trail:
        trail_lines.append(
            f"  R{t['round']}: 순자산={t['networth_pct']:.2%}, "
            f"공포={t['fear']}, 탐욕={t['greed']}, "
            f"행동={t['lastAction']}, 시장공탐지수={t['market_fg']:.0f}"
        )
    trail_summary = "\n".join(trail_lines) if trail_lines else "  (기록 없음)"

    return f"""아래는 시장 시뮬레이션 게임에서 "{agent.alias}" (유형: {agent.type})의 10라운드 플레이 기록이다.

## 라운드별 기록
{trail_summary}

## 종합 상태
- 최종 순자산 변화율: {arc_state['networth_pct']:.2%}
- 최저 순자산: {arc_state['min_networth_pct']:.2%}
- 최고 공포: {arc_state['peak_fear']:.0f}, 최종 공포: {arc_state['final_fear']:.0f}
- 최고 탐욕: {arc_state['greed_peak']:.0f}
- 매수 경험: {arc_state['ever_bought']}, 대량매수: {arc_state['ever_bought_large']}
- 공포장 매수: {arc_state['bought_in_fear']}, 탐욕장 매수: {arc_state['bought_in_greed']}
- 차익실현: {arc_state['took_profit']}

## 엔딩 선택지
{options}

위 기록을 바탕으로 이 캐릭터에게 가장 어울리는 엔딩 하나를 골라라.
반드시 아래 JSON 형식으로만 응답하라:
{{"ending_id": "..."}}"""


def _select_ending_llm(
    client, agent: Agent, arc_state: dict, trail: list[dict]
) -> str | None:
    """Ask the LLM to pick an ending. Returns ending_id or None on failure."""
    from .llm import safe_json
    prompt = _build_llm_prompt(agent, arc_state, trail)
    result = safe_json(
        client, prompt,
        fallback={},
        system="너는 시장 시뮬레이션 게임의 스토리 디렉터다. 캐릭터의 행동 기록을 보고 가장 적합한 엔딩을 선택한다.",
        temperature=0.3,
    )
    eid = result.get("ending_id")
    valid = _TYPE_ENDINGS.get(agent.type, [])
    if eid in valid:
        return eid
    return None
...
class ArcTracker:
    ...
    def endings(self, agents: list[Agent], client=None) -> list[EndingResult]:
        """Determine endings for all protagonists.

        If ``client`` (LLMClient) is provided, the LLM picks each ending based
        on the full arc trail. Falls back to gate-based heuristics on LLM
        failure or when no client is given.
        """
        ...
        for a in agents:
            ...
            trail = self.trail.get(a.id, [])

            # Try LLM first, fall back to gates
            ending_id = None
            if client is not None:
                ending_id = _select_ending_llm(client, a, s, trail)
