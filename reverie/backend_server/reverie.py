...
class ReverieServer: 
    ...
    while (True): 
      ...
      if check_if_file_exists(curr_env_file):
        ...
        if env_retrieved: 
          ...
          # The forked base sim may not ship a movement/ directory; make sure it
          # exists before we write this step's movement file.
  ...
  def open_server(self): 
    ...
    while True:
      ...
      try:
        ret_str, should_break = self.command(sim_command)
        print (ret_str)
        if should_break:
          break
  ...
  def command(self, sim_command):
    """
    Execute a single console command -- the same command set accepted by the
    interactive open_server() prompt -- and return (ret_str, should_break).

    This is factored out of open_server() so that non-interactive front-ends
    (e.g., the web control server) can drive the simulation with the exact
    same commands the terminal accepts.

    INPUT
      sim_command: the raw command string (e.g., "run 100", "save",
                   "print persona schedule Isabella Rodriguez")
    OUTPUT
      (ret_str, should_break): <ret_str> is the textual output of the command;
                               <should_break> is True for commands that end the
                               session ("fin"/"exit").
    """
